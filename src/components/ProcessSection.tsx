...
export const ProcessSection: React.FC<ProcessSectionProps> = ({
  ...
  return (
    <section id="process" className="relative py-20 bg-[#020617] border-t border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-14">
          {WORKFLOW_PHASES.map((phase) => {
            ...
            return (
              <div
                key={phase.phase}
                className="group relative p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-blue-500/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 shadow-lg text-left flex flex-col justify-between"
              >
                <div>
                  {/* Phase Number & Icon */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-10 h-10 rounded-xl bg-blue-500/20 text-blue-400 border border-blue-500/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                      <Icon className="w-5 h-5" />
                    </div>
                    <span className="font-tech text-xs font-black text-slate-500 group-hover:text-blue-300">
                      0{phase.phase}
                    </span>
                  </div>

                  <h3 className="font-display font-bold text-base text-white group-hover:text-cyan-300 transition-colors mb-2">
                    {phase.title}
                  </h3>
                ...
                <div className="pt-3 border-t border-white/10">
                  <span className="font-tech text-[10px] text-slate-500 uppercase tracking-wider block mb-1.5">
                    Key Outcomes:
                  </span>

...
export const TrainingSection: React.FC<TrainingSectionProps> = ({
  ...
  return (
    <section id="training" className="relative py-20 bg-slate-50 dark:bg-[#020617] border-t border-slate-200 dark:border-white/5 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-14">
          {TRAINING_HIGHLIGHTS.map((item, idx) => (
            <div
              key={idx}
              className="p-3.5 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-blue-500/50 shadow-sm backdrop-blur-sm transition-all text-center flex flex-col items-center justify-center"
            >
              <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 flex items-center justify-center mb-2">
                <CheckCircle2 className="w-4 h-4" />
              </div>
              <h4 className="text-xs font-bold text-slate-900 dark:text-slate-200 mb-1">{item.title}</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((course) => {
            ...
            return (
              <div
                key={course.id}
                className="group rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-blue-500/50 p-5 sm:p-6 transition-all duration-300 hover:-translate-y-1 shadow-sm dark:shadow-lg flex flex-col justify-between backdrop-blur-sm"
              >
                <div>
                  {/* Category & Level Badges */}
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-700 dark:text-blue-300 text-[10px] font-tech font-bold uppercase tracking-wider">
                      <CategoryIcon className="w-3 h-3" />
                      <span>{course.category}</span>
                    </div>

                    <span className="text-[11px] font-tech font-semibold text-slate-500 dark:text-slate-400">
                      {course.level}
                    </span>
                  </div>

                  <h3 className="font-display font-bold text-lg text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-cyan-300 transition-colors mb-2">
                    {course.title}
                  </h3>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4 p-2.5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-[11px]">
                  ...
                  <div className="mb-4">
                    <button
                      ...
                      className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-300 flex items-center justify-between py-1 border-b border-slate-200 dark:border-white/10"
                    >
                    ...
                    {isExpanded && (
                      <div className="mt-2 space-y-1.5 pt-1 animate-in fade-in duration-200">
                        {course.syllabus.map((modItem, idx) => (
                          <div
                            key={idx}
                            className="flex flex-col text-[11px] text-slate-700 dark:text-slate-300 pb-1 border-b border-slate-100 dark:border-white/5"
                          >
                            <span className="text-blue-600 dark:text-blue-400 font-tech font-bold text-[10px]">
                              {modItem.module}
                            </span>
                            <span className="text-slate-500 dark:text-slate-400 text-[11px]">{modItem.topics.join(' • ')}</span>
                          </div>
                        ))}
                      </div>
                    )}
        ...
        <div className="mt-14 p-6 sm:p-8 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-sm shadow-md dark:shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 text-left">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 text-xs font-tech font-bold uppercase">
          ...
          <button
            ...
            className="px-6 py-3 rounded-full bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-300 dark:border-white/10 hover:border-blue-500/50 text-slate-800 dark:text-slate-200 text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all shadow-sm"
          >

...
export const ServicesSection: React.FC<ServicesSectionProps> = ({
  ...
  return (
    <section id="services" className="relative py-20 bg-[#020617] border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {DIVISIONS.map((div) => {
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl text-left transition-all duration-300 flex flex-col justify-between border backdrop-blur-sm ${
                  isActive
                    ? 'bg-blue-500/10 border-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.3)] ring-1 ring-blue-400/40'
                    : 'bg-white/5 border-white/10 hover:border-blue-500/40 hover:bg-white/10 text-slate-300'
                }`}
              >
                <div className="flex items-center justify-between mb-2">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                      isActive ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-white/5 text-slate-400 border border-white/5'
                    }`}
                  >
                    <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <span className="font-tech text-xs font-bold text-slate-500">
                    {div.number}
                  </span>
                </div>
                <div>
        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="max-w-2xl text-left">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-tech text-xs font-bold text-blue-400 tracking-wider uppercase">
                Category {currentDivision.number}
              </span>
              <span className="text-white/20">•</span>
              <span className="text-xs text-slate-300">{currentDivision.services.length} Specialized Services</span>
            </div>
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-full bg-white/5 border border-white/10 focus:border-blue-400 focus:outline-none text-xs text-slate-200 placeholder:text-slate-500 backdrop-blur-sm"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {displayedServices.map((service) => (
            <div
              key={service.id}
              className="group relative p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-blue-500/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 shadow-md flex flex-col justify-between"
            >
              <div>
                {/* Badge if popular */}
                <div className="flex items-center justify-between mb-3">
                  {service.popular ? (
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-500/15 text-blue-300 border border-blue-500/30 text-[10px] font-tech font-bold uppercase tracking-wider">
                      <Sparkles className="w-2.5 h-2.5" /> High Demand
                    </span>
                  ) : (
                    <span className="text-[11px] font-tech text-slate-500 uppercase tracking-wider">
                      Specialized
                    </span>
                  )}
              ...
              <div className="pt-4 border-t border-white/10 flex items-center justify-between gap-2">
                <button
                  ...
                  className="text-xs font-semibold text-blue-400 hover:text-cyan-300 flex items-center gap-1 group/btn"
                >

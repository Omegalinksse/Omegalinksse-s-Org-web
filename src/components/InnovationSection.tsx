...
export const InnovationSection: React.FC<InnovationSectionProps> = ({
  ...
  return (
    <section id="innovation" className="relative py-20 bg-slate-50 dark:bg-[#020617] border-t border-slate-200 dark:border-white/5 overflow-hidden transition-colors">
      {/* Background Tech Image Blend */}
      <div className="absolute inset-0 z-0 opacity-5 dark:opacity-15 pointer-events-none">
        <img
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-slate-50/90 via-slate-50/80 to-slate-50 dark:from-[#020617] dark:via-[#020617]/85 dark:to-[#020617]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          {INNOVATION_INITIATIVES.map((init, idx) => {
            ...
            return (
              <div
                key={idx}
                className="group relative p-6 sm:p-7 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-blue-500/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 shadow-sm dark:shadow-xl text-left flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-12 h-12 rounded-2xl bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 flex items-center justify-center group-hover:scale-110 transition-transform">
                      <Icon className="w-5 h-5" />
                    </div>

                    <span className="px-2.5 py-1 rounded-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-[10px] font-tech text-blue-700 dark:text-blue-300 uppercase font-bold tracking-wider">
                      {init.stage}
                    </span>
                  </div>

                  <h3 className="font-display font-bold text-xl text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-cyan-300 transition-colors mb-2">
                    {init.title}
                  </h3>
                ...
                <div>
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-white/10">
                    <div className="text-[10px] font-tech text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {init.focus.map((f, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-white/10 text-[11px]"
                        >
                          {f}
                        </span>
                      ))}
        ...
        {/* Callout Box */}
        <div className="p-6 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-sm shadow-sm dark:shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4 text-left">
